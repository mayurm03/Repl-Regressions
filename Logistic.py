...
def plotData(X,y):
  # plots the data points with o for the positive examples and x for the negativ examples. output is saved to file graph.png
...
def computeCost(X, y, theta):
  ...
  m = len(X)
  k = -y*np.dot(X,theta)
  cost = (1/m)*(np.sum(np.log(1+np.exp(k))))
  return cost
...
def computeGradient(X,y,theta):
 ...
 # pred = predict(X,theta)
  k = (-y*np.dot(X,theta))
  k = -y*(np.exp(k) / (1+np.exp(k)))
  for i in range(num):
    exp = np.sum(np.multiply(k,X[:,i]))
    m = (X[:,i]).size
    grad[i] =  exp/m
  return grad
